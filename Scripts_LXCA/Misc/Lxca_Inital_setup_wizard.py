...
import time, unittest, os, xmlrunner
#import flexcatSetup
...
class Lxca_Initial_Setup_Test(unittest.TestCase):
    ...
    def test_009_recovery_password(self):
        self.driver.find_element_by_id('ServiceRecoveryPasswordpasswordRecoveryNode').send_keys('IFM44test')
        self.driver.find_element_by_xpath('//span[3][contains(text(),"Apply")]').click()
        self.assertTrue(self.exCheck('//img[contains(@src,"/ui/lxca/customUI/logs/initialSetup/images/ac16_passwordhide_24.png")]'))
        tmp = self.driver.find_elements_by_xpath('//span[3][contains(text(),"Close")]')
        elem= self.isDisplayed(tmp)
        elem.click()
        


    def test_010_setup_Service_and_Support(self):
